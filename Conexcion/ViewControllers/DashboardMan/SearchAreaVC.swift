...
class cellSearchArea: UITableViewCell {
...
class SearchAreaVC: UIViewController {
    ...
    @objc func btnBlockAction(_sender:UIButton) {
        ...
        let Dic =  RecentSearchList(fromDictionary: Dict as! [String : Any])
        let strTitle = Dic.friendRequestStatus  //friendRequestStatus
        if strTitle == ""{
    ...
    @objc func WebServiceForGetNotificationList() {
        ...
        let parameter: [String: Any] = [
            API_UserId:UserId,
            STATUS:"acceptDecline"
        ]
        print("request params: \(parameter)")
        MBProgressStart()
        APIManager.callAPIBYPOST(parameter: parameter, url: ApiBaseURL + GetUserFriendMaleList, OnResultBlock: { (response, status) in
            print("------\(response)------")
            ...
            if status == "true" {
                ...
                print(dictData ?? "")
                if dictData != nil{
                    let arrRes = dictData?.value(forKey: DATA) as? NSArray
...
extension SearchAreaVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:cellSearchArea = tableView.dequeueReusableCell(withIdentifier: "cellSearchArea") as! cellSearchArea
        ...
        let isBlock = Dic.friendRequestStatus
        if isBlock == "accept"{
        ...
        cell.btnInfo.addTarget(self, action: #selector(SearchAreaVC.btnInfoAction(_sender:)), for: .touchUpInside)
        cell.btnBlock.addTarget(self, action: #selector(SearchAreaVC.btnBlockAction(_sender:)), for: .touchUpInside)
        return cell
    }
